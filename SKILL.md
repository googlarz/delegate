...
Before anything else, check whether a matching `[job-name]/` folder already exists (the current project's `.claude/skills/`, then `~/.claude/skills/`). If found, read its `SKILL.md` and `runs.md` and continue from there instead of restarting. Check these in order, top to bottom, and act on the first one that matches:
...
This is separate from two other standing checkpoints, neither of which is a pipeline-continuation pause and neither of which counts against or adds to the "two exceptions" above: the irreversible-action gate (step 3, and its batched form in "Running: plan, recurse, parallelize"), which fires every time a send/publish/delete/overwrite is about to happen, run or not; and the one-line Definition-of-Done confirmation (step 5), shown once when the check list is first finalized for a job, never again on later runs of the same job.
...
The expanded-in-progress mark is literal text appended to that Step's line in SKILL.md's Steps section — `(expanding...)` — added the moment you start decomposing it. Remove it as soon as the decomposition finishes executing successfully, whether or not the sub-plan turns out to be stable enough to write back (below) — a one-off sub-plan still clears the mark, it just doesn't get persisted. Only leave the mark in place if you stop mid-decomposition, so Step 0 can find it and resume. A Step showing this suffix has no other required syntax; the sub-plan itself is just nested numbered/lettered lines under it once written.

For Definition-of-Done and `runs.md` purposes (step 5), a decomposed Step is done only when every one of its sub-steps succeeds. `runs.md` still has no row type but "check" (or "check per item" for batches) — a Step doesn't get its own row. Instead, if any sub-step of a Step fails its first pass, treat it as a first-pass failure of every Definition-of-Done check that verifies that Step's output, and name the specific failing sub-step in that check's evidence field — don't let a sub-step failure get absorbed into a passing check verdict.

If a discovered sub-plan is stable — structure that will recur on the next run, not one-off detail from this run's specific input — write it back into SKILL.md's Steps as nested sub-steps (the mark is already gone by this point, per above), so future runs start from the plan already found instead of re-deriving it. Do this after step 5's checks pass for the run (a sub-plan is only worth persisting once verified to work); on a job's very first run, this happens in the same pass as step 5 replacing the Definition-of-Done placeholder — the two edit different sections of SKILL.md (Steps vs. Definition of done) and don't conflict. Note in the run report the first time a step gets expanded this way.
